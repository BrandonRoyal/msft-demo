...
docker run --name redis --network default -d brandonroyal/redis:latest
...
docker run --name nginx --network default -p 80:80 -d brandonroyal/nginx:latest powershell ./start.ps1 $web_ip $api_ip
Write-Host "------"